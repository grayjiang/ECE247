...
class SVM(object):
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
        ...
        loss += (sum(c)-1)
        b[y[i]] = 0
        b = b.reshape(num_classes,1)
        d = X[i].reshape(1,self.W.shape[1])
        grad_temp = b.dot(d)
        grad_temp[y[i]] = -1 * sum(grad_temp)
        grad += grad_temp
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    a = self.W.dot(X.T)
    pick = np.choose(y,a) 
    pick = pick.reshape(num_train,1)
    b = a.T - pick+1
    c = (b>0).astype(int)
    d = c * b
    loss = sum(sum(d)) / num_train - 1
    ...
    # ================================================================ #
    c[np.arange(num_train),y] = 0 
    rows = np.sum(c, axis=1)
    c[np.arange(num_train), y] = -rows.T
    grad = (c.T).dot(X)
    grad /= num_train
