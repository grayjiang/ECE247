...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    out_1 = X.dot(W1.T) + b1.reshape(1,-1)
    a = (out_1>0).astype(int)
    out_ru = a * out_1
    scores = out_ru.dot(W2.T) + b2.reshape(1,-1)
    ...
    scores_sum = sum(scores_e.T).T
    data_loss = sum(np.log(scores_sum)) - sum(scores[np.arange(N),y])
    data_loss /= N
    ...
    b2 = scores_e / scores_sum.reshape(-1,1)
    b2[np.arange(N), y] -= 1
    dru = b2.dot(W2) * a
    
    grads['W2'] = (b2.T).dot(out_ru)/N    
    grads['W2'] += reg * W2
    grads['b2'] = np.sum(b2, axis=0)/N 
    grads['W1'] = (dru.T).dot(X)/N 
    grads['W1'] += reg * W1
    grads['b1'] = np.sum(dru, axis=0) / N      
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # YOUR CODE HERE:
  	  ...
  	  # ================================================================ #
        item = np.random.choice(np.arange(num_train),batch_size)
        X_batch = X[item]
        y_batch = y[item]
